function mCADload(obj)
% Load Pioneer P3-Dx 3D CAD model

obj.pCAD.obj{1} = loadawobj('Munix_leve.obj');
obj.pCAD.mtl{1} = loadawmtl('Munix_leve.mtl');

% Flags
obj.pCAD.flagLoaded  = 1;  % 3D image loaded
obj.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
obj.pCAD.flagLines   = 1;  % Illustrate lines between and surface
obj.pCAD.flagpPlot = 1;

% Ajustando a orientação e posição iniciais:
obj.pCAD.obj{1}.v = [1 0 0; 0 0 -1; 0 1 0]*obj.pCAD.obj{1}.v;%90 graus em X
obj.pCAD.obj{1}.v = [0 1 0; -1 0 0; 0 0 1]*obj.pCAD.obj{1}.v;%90 graus em X
obj.pCAD.obj{1}.v(1,:) = obj.pCAD.obj{1}.v(1,:) - 0*ones(1,length(obj.pCAD.obj{1}.v(1,:)));
obj.pCAD.obj{1}.v(2,:) = obj.pCAD.obj{1}.v(2,:) + 0*ones(1,length(obj.pCAD.obj{1}.v(2,:)));
obj.pCAD.obj{1}.v(3,:) = obj.pCAD.obj{1}.v(3,:) - min(obj.pCAD.obj{1}.v(3,:)+0.0)*ones(1,length(obj.pCAD.obj{1}.v(3,:)));
